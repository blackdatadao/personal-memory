...
def query_message(
    ...
    introduction = '用下面的文章回答问题后面的问题。如果在文章中找不到相关答案，请回答"我不知道. '
    question = f"\n\nQuestion: {query}"
...
def ask(
    ...
    if print_message:
        print(message)
    messages = [
        {"role": "system", "content": "你是冀田，用第一人称、冀田的口吻在详细、友善、有条理的回答问题。"},
...
df = pd.read_csv("example_data/timothy_stream.csv")
...
# input box
user_input = st.text_input("你", "你的投资理念是什么？")
# button
if st.button("Send"):
    # display user input
